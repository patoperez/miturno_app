...
- **UN BOTÓN DE GUARDAR QUE NO HACE NADA ES EL PEOR FALLO POSIBLE.** El usuario culpa a la función ("la app no me deja registrar el pasado") en vez del campo que falta, y se va. Esto **ya no es una convención que haya que recordar** — las convenciones que dependen de la memoria son justo lo que produjo ese bug. Lo garantiza `sheet()`:

### `sheet(inner, msg)` — la hoja viene armada
`sheet()` pasa el contenido por **`armSheet(inner, msg)`**, que hace tres cosas que el autor de la hoja **no puede omitir porque no son suyas**:
1. **Mete la ranura de aviso** (`<div class="fmsg" id="fmsg">`) **justo encima del primer botón principal** — donde el usuario ya está mirando, no arriba fuera de cuadro. Si no hay botón principal la pone encima del primero que haya. **No existe forma de escribir una hoja sin ranura.**
2. **Enruta cada botón principal por `submitSheet`**, reescribiendo su `onclick`.
3. **Le quita `disabled`** a los botones principales. Un botón apagado (`.btn.p.off`) **sigue siendo tocable a propósito**: al tocarlo explica qué falta. Un `disabled` de verdad no puede decir por qué, que es exactamente el problema que esto resuelve. Por eso el framework no lo permite.

**`formError(msg)`** pinta un `.ferr` rojo en la ranura y devuelve `false`, para escribir `return formError("…")` de una línea. **No recibe a dónde escribir**: solo hay una hoja abierta a la vez, así que la ranura tiene un id fijo (`FMSG_ID`) y no se puede apuntar al slot equivocado ni olvidar el parámetro. `futureDateMsg()` es un `formError` con el texto fijo `MSG_FUTURO`.

**`submitSheet(fn)`** es la red de seguridad: limpia la ranura, corre el handler y, **si la hoja quedó exactamente igual y nadie escribió un aviso, escribe `MSG_SIN_RAZON`**. Un guardado que funciona cierra la hoja, y repintarse también la cambia, así que no hay falsos positivos. Un handler nuevo que se olvide de explicar produce un aviso genérico, nunca silencio.

**Escribir una hoja: solo el contenido.** Los botones van como siempre (`<button class="btn p" onclick="saveX()">`), con **la clase pegada a `<button` como primer atributo** — así es como `armSheet` los reconoce, y hay una prueba que lo fija. `msg` es el segundo parámetro y solo hace falta en las hojas que **se repintan solas** (el alta manual de entreno): el aviso viaja en el estado (`_MW.err`) y **se consume al pintarlo**, para que no se quede pegado al cambiar el día o la rutina.

**Todo aviso pasa por la ranura.** No hay áreas de mensaje ad-hoc: los importadores (plan, comidas, rutinas, respaldo) también usan `formError`. `applyBackup` escribe ahí su porqué — restaurar un respaldo inválido era de los peores sitios donde quedarse callado.
- UI en español.
...
Para los avisos de formulario: que guardar un entreno manual **sin series** deje un `.ferr` visible y **no** guarde nada, que agregarle una serie limpie el aviso y sí guarde, y que ninguna guarda (proyecto, lead, idea, número, tarea, comida, actividad, meta, sesión, renombrar ejercicio) aborte sin dejar texto en la ranura.
Para la garantía de `sheet()` (suite propia): que `armSheet` ponga **una** ranura y **encima** del botón principal, que enrute todo botón principal por `submitSheet` sin tocar los secundarios, que le quite `disabled`, que no envuelva dos veces, y que escape el aviso pendiente. Que `submitSheet` escriba `MSG_SIN_RAZON` cuando el handler deja la hoja igual y muda, y que **no** lo haga cuando cerró, repintó o ya explicó. Y por escaneo del código: que nadie pinte una `.sheet` por fuera de `sheet()`, que nadie escriba la ranura a mano, que `formSlot` ya no exista, que no queden áreas de mensaje ad-hoc, y que **cada una de las 54 hojas** se abra con su ranura y sus botones enrutados — la lista tiene que cubrirlas todas, así que **agregar una hoja nueva sin cubrirla hace fallar la suite**. En el navegador se **tocan** los botones de verdad (el `onclick` reescrito solo corre ahí): ningún formulario puede quedar mudo.
